...
begin
  ...
  { intros p hp,
    ...
    rw ←tsub_lt_iff_left ha1 at ha2,
    rw ←tsub_lt_iff_left hb1 at hb2,
    split,
...
begin
  ...
  norm_cast,
  rw ←nat.le_div_iff_mul_le (dec_trivial : 2 > 0),
  norm_num,
  rw sub_eq_zero,
  norm_cast
end
...
begin
  ...
  { intros p hp,
    ...
    split,
    { exact (tsub_lt_iff_left hpa).mpr hpa2 },
...
begin
  ...
  { conv_lhs { rw nat.succ_eq_add_one },
    ...
    rw hx _ _,
    { simp_rw row_cols_alternate_colour c a (nat.succ x) hk2 halt x (nat.lt_succ_self x),
      conv_rhs { rw nat.succ_eq_add_one x },
      by_cases h : a ∈ c; by_cases he : even x,
      { simp_rw ←nat.even_iff,
        rw [if_pos h, if_pos h],
        simp_rw iff_true_intro (iff_of_true h he),
        rw if_true,
...
begin
  ...
  { simp_rw ←hnalt1,
    ...
    { simp [h, (show 2 * k2 + 1 + 1 = 2 * (k2 + 1), by ring)],
      ring },
    { rw [add_comm (2 * k2), nat.add_mul_div_left 1 k2 (show 2 > 0, by norm_num)],
      norm_num,
      simp [h] } },
  { linarith },
  { exact row_cols_alternate_ends_middle_alt c a k2 hk2 halt }
  ...
  { by_cases hnalt : ∃ k : ℕ, 0 < k ∧ k < 2 * (nat.succ k2a) + 1 ∧
    ...
    { rcases hnalt with ⟨k, hk0, hk1, halt⟩,
      ...
      { set k3 := k / 2 with hk3,
        ...
        have hk4b : k4 ≤ k2a,
        { rw [hk4, nat.succ_eq_add_one, tsub_le_iff_left],
          linarith only [hk3c] },
  ...
  { intro h,
    ...
    { intro k,
      ...
      { intro hst,
        ...
        simp_rw [mem_filter, mem_Ico, iff_true_intro (zero_le _), iff_true_intro (nat.succ_pos _),
                 iff_false_intro (nat.succ_ne_zero _), true_and, false_and, false_or],
        erw mem_filter at h,
        cases h with hcu h,
        unfold row_cols_alternate_at_parity at h,
        by_cases hpar : t % 2 = parity,
        { rw nat.succ_eq_add_one t,
          have hht := (h t (by linarith only [hst]) hpar).not_left,
          interval_cases parity; simp [hpar, hht] },
        { simp_rw [iff_false_intro hpar, false_or, nat.left_distrib],
  ...
  { have ha : (((2 * a - 1 - parity : ℕ) : fin (n + 1)) ∈ map_to_alt_colouring (n + 1) parity x ↔
    ...
    { have hage : n + 1 ≤ 2 * b + (1 - parity) + 1, { linarith },
      ...
      { intro hax,
        ...
        rw nat.le_div_iff_mul_le (show 2 > 0, by norm_num) at hax2b2,
      ...
      { intro hay,
        ...
        rw nat.le_div_iff_mul_le (show 2 > 0, by norm_num) at hay2b2,
