...
begin
  ...
  { apply fin.eq_of_veq,
    unfold fin.val,
    exact coe_val_of_lt (show a_val + 1 < n + 1, by linarith only [ha]) },
  have hbo : ((b_val + 1 : ℕ) : fin (n + 1)) = ⟨b_val + 1, hbo1⟩,
  { apply fin.eq_of_veq,
    unfold fin.val,
    exact coe_val_of_lt (show b_val + 1 < n + 1, by linarith only [hb]) },
...
begin
  ...
  { by_cases h : b.val < m,
    { rw nat.succ_eq_add_one at hb2a,
      ...
      { rw coe_coe_of_lt (show m < n + 1, by linarith),
        linarith },
      convert rows_alternate_two_cols c hbal2 a (m : fin (n + 1)) ha hb2ax,
      { exact (coe_coe_of_lt (show m < n + 1, by linarith)).symm },
      { exact (coe_coe_of_lt (show m < n + 1, by linarith)).symm } },
...
begin
  have hb2b : b2 = ((b2 : ℕ) : fin (n + 1)) := (coe_val_eq_self b2).symm,
...
begin
  ...
  { rw ←not_iff_not,
    ...
    have hf : ((k : fin (n + 1)) : ℕ) = k :=
      coe_coe_of_lt (show k < n + 1, by linarith only [ha]),
...
begin
  ...
  { repeat { rw fin.coe_eq_val },
    rw fin.coe_eq_val at ha,
    exact coe_val_of_lt (show a.val + 1 < n + 1, by linarith) },
...
begin
  ...
  { set k2 := (a2 - a1) / 2 with hk2,
    use (a1 : fin (n + 1)),
    use k2,
    rw coe_coe_of_lt (show a1 < n + 1, by linarith),
  ...
  { set k2 := (a1 - a2) / 2 with hk2,
    use (a2 : fin (n + 1)),
    use k2,
    rw coe_coe_of_lt (show a2 < n + 1, by linarith),
...
begin
  ...
  { intro h,
    ...
    { intros k hk hpar,
      ...
      { rw ha,
        symmetry,
        exact coe_coe_of_lt (show k < n + 1, by linarith only [hk]) },
      rw hk2,
      rw hk2 at hk,
      exact h a hk },
    { intros k hk hpar,
  ...
  { intro h,
    ...
    { set k := (a : ℕ) with hk,
      have hk2 : a = (k : fin (n + 1)) := (coe_val_eq_self a).symm,
      rw hk2,
      exact h0 k ha hpar },
    { rw [←nat.even_iff, nat.not_even_iff] at hpar,
      set k := (a : ℕ) with hk,
...
begin
  ...
  { have ha : (((2 * a - 1 : ℕ) : fin (2 * n + 1)) ∈ map_to_alt_colouring (2 * n + 1) x ↔
    ...
    by_cases hble : b + 1 ≤ n,
    { rw [coe_coe_of_lt (show 2 * b + 1 < 2 * n + 1, by linarith only [hble]),
          coe_coe_of_lt (show 2 * b + 1 + 1 < 2 * n + 1, by linarith only [hble])] at ha,
...
begin
  ...
  { rw [nat.sub_self, zero_add, coe_coe_eq_self a],
    simp },
  { rw fin.coe_eq_val,
    linarith [a.is_lt] }
end
...
begin
  ...
  { intro hc1,
    ...
    { unfold row_alt_colourings_1 at hc1,
      ...
      { intros k hk hpar,
        unfold reverse_row_colouring,
        erw [mem_filter, mem_filter],
        rw [coe_coe_of_lt (show k < 2 * (n + 1) + 1, by linarith only [hk]),
            coe_coe_of_lt (show k + 1 < 2 * (n + 1) + 1, by linarith only [hk]),
  ...
  { intro hc0,
    ...
    { intros k hk hpar,
      ...
      erw [mem_filter, mem_filter],
      rw [coe_coe_of_lt (show k < 2 * (n + 1) + 1, by linarith only [hk]),
          coe_coe_of_lt (show k + 1 < 2 * (n + 1) + 1, by linarith only [hk]), add_comm k 1,
...
begin
  ...
  { intros k2 hk2,
    ...
    { exact (coe_coe_of_lt (show k2 < n + 1, by linarith)).symm },
    { rw coe_coe_of_lt (show k2 < n + 1, by linarith),
      linarith } },
  { rw hk,
    exact a.is_lt }
end
  ...
  { intros i hi,
    ...
    { intros a ha,
      ...
      { intro h,
        ...
        rw coe_coe_of_lt (show (a : ℕ) + 1 < n + 1, by linarith only [ha]) at ha1i,
        rw ←hai at ha1i,
      ...
      { intro h,
        ...
        use mem_univ _,
        rw coe_coe_of_lt (show (a : ℕ) + 1 < n + 1, by linarith only [ha]) at h,
        by_cases hpar : i % 2 = 0,
