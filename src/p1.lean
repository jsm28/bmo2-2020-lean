...
begin
  ...
  { right,
    ...
    have hnd2: n / b < 2, by exact int.div_lt_of_lt_mul hbpos hnupper,
    have hnd1: n / b ≤ 1,
...
theorem mod_mul_2 (n : ℤ) (a : ℤ) (b : ℤ) (hbpos : 0 < b) (hmod : n % b = a) :
    n % (2 * b) = a ∨ n % (2 * b) = a + b :=
begin
  have hdvd: b ∣ 2 * b, by exact dvd_mul_left _ _,
  ...
  have hn2bupper: n % (2 * b) < 2 * b, by exact mod_lt_of_pos _ h2bpos,
...
begin
  have h2m1: (2^(m+1) : ℤ) = (2 * 2^m : ℤ), by refl,
  rw h2m1,
  ...
  { exfalso,
    ...
    { have hm2: ∃ m2 : ℕ, m = 2 + m2, by exact nat.exists_eq_add_of_le hm,
    ...
    have hbb: (2 + 2 * 2 ^ m1 * (1 + 2 * d)) = 2 * (1 + 2^m1 * (1 + 2 * d)), by ring,
    ...
    have hbbx: (3 + (2 : ℤ)^m1) % 2^m = 3 % 2^m, by rw [← mod_mod, hb],
    ...
    rw sub_self at heqx,
    have heqxx: 2 * (2^m1 : ℤ) - 2^m1 = 2^m1, by ring,
    rw heqxx at heqx,
    have heqz: 0 < (2^m1 : ℤ), by exact power_of_2_pos m1,
    rw ← heqx at heqz,
...
begin
  ...
  let an : ℤ := a n,
  let an1 : ℤ := a (n + 1),
  have han1 : an1 = a (n + 1), by refl,
  have hrecn : a (n + 1) = p1_seq_next an, by rw hrec n,
  rw ← han1 at hrecn,
  have hanodd : odd an, by exact hodd n,
  have han1odd : odd an1, by exact hodd (n + 1),
  rw hrecn at han1odd,
...
begin
  ...
  have hrecn : a (n + 1) = p1_seq_next an, by rw hrec n,
  rw ← han1 at hrecn,
  have hanmod : an % 2^m = 3, by exact hmod n,
  have han1mod : an1 % 2^m = 3, by exact hmod (n + 1),
  rw hrecn at han1mod,
...
begin
  intros m n,
  by_cases h2m : 2 ≤ m,
  { have hmod : (a n) % 2^m = 3, by exact three_mod_all_powers a hodd hrec m h2m n,
    rw mod_def at hmod,
    have hmodx : a n - 2^m * (a n / 2^m) + 2^m * (a n / 2^m) = 3 + 2^m * (a n / 2 ^ m), by rw hmod,
  ...
  { have hmle1: m ≤ 1,
    { apply le_of_not_gt,
      rw gt,
      exact h2m, },
    have haodd: a n % 2 = 1, by exact hodd n,
    rw mod_def at haodd,
    ...
    have ha3d2 : 2 ∣ a n - 3, by exact dvd_of_mul_right_eq _ ha3m2.symm,
    rw (show (2 : ℤ) = 2^1, by norm_num) at ha3d2,
    have hm1div: (2 : ℤ)^m ∣ 2^1,
    { have hm1: ∃ m1 : ℕ, 1 = m + m1, by exact nat.exists_eq_add_of_le hmle1,
...
begin
  by_cases haeq0: a = 0,
  { assumption },
  { have ha : 2^a ∣ a, by exact h a,
    have h2pself : ∀ n : ℕ, n < 2^n,
    { intro n,
      exact nat.lt_pow_self (show 1 < 2, by norm_num) n, },
    exfalso,
    have h2aself: a < 2^a, by exact h2pself a,
    have hapos : a > 0, by exact nat.pos_of_ne_zero haeq0,
    have hself2a : 2^a ≤ a, by exact (nat.le_of_dvd hapos) ha,
    have haa: a < a, by exact lt_of_lt_of_le h2aself hself2a,
...
begin
  ...
  have ha0: 0 ≤ a 0 - 3,
  { linarith, },
  have ha30: a 0 - 3 = 0, by exact zero_if_all_powers_divide_int (a 0 - 3) ha0 hallpowers,
